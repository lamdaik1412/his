...
function khoitao() {
	$.get(
		window.location.origin +
			"/web_his/ds_noitiepnhan?_search=false&nd=1547738873909&rows=200000&page=1&sidx=&sord=asc"
	).done(function (d) {
		dmnoidangky = d;
		for (i = 0; i < dmnoidangky.length; i++) {
			dmndk[dmnoidangky[i].MA_NOITIEPNHAN] =
				dmnoidangky[i].TEN_NOITIEPNHAN;
		}
	});

	$("#tabtiepnhan").append(
		'<div id="mod_bhyt_section"><div id="mod_bhyt_btn" style="float:right;"></div></div><div style="clear:both; margin: 5px; text-align: center;" id="mod_bhyt_message"><span id="statusMessage"><span id="part1" style="font-size:16px;"></span><span id="part2" style="color: rgb(0, 112, 192); font-weight:bold;text-align: left;"></span><span id="part3" style="color: rgb(0, 176, 105);"></span></span><div style="clear:both; margin-top: 0px; text-align: center;" id="mod_bhyt_lichsu_msg" /><div><input id="tmpnamsinh" style="display:none;"/><input id="tmptungay" style="display:none;"/><input id="tmpdenngay" style="display:none;"/><input id="tmpthoigian_du5namlientuc" style="display:none;"/></div>'
	);

	var script = document.createElement("script");
	script.innerHTML =
		'$("#tmpnamsinh").on("click",function (){if($("#tmpnamsinh").val()!=""){$("#namsinh").val($("#tmpnamsinh").val());$("#namsinh").change();};if($("#tmptungay").val()!=""){$("#tungay").val($("#tmptungay").val());$("#tungay").change();};if($("#tmpdenngay").val()!=""){$("#denngay").val($("#tmpdenngay").val());$("#denngay").change();};if($("#tmpthoigian_du5namlientuc").val()!=""){$("#thoigian_du5namlientuc").val($("#tmpthoigian_du5namlientuc").val());$("#thoigian_du5namlientuc").change();};});';
	document.head.appendChild(script);

	$("#baohiem5nam_label")
		.parent()
		.append(
			'<button id="mod_check_bhyt_hai" style="margin:3px; padding:5px; border-radius:2px;font-weight:bold;" type="button" class="btn_bh"><span class="glyphicon glyphicon-search"></span>Kiểm tra BHYT</button>'
		);

	$("body").append(
		'<div id="mod_lichsuct" style="display:none;"><h1>Lịch sử 10 lần gần nhất (click bất kỳ trong bảng để thoát)</h1><table id="mod_lichsukcb"></table><table id="mod_lichsukt"></table></div>'
	);
	//ham xu ly
	$("#mod_check_bhyt_hai").on("click", function () {
		if (
			($("#sobhyt").val().length == 15 ||
				$("#sobhyt").val().length == 10) &&
			$("#hoten").val() != ""
		)
			checkthe();
	});
	$("#sobhyt").keypress(function (e) {
		if (
			e.keyCode == 13 &&
			$("#hoten").val() != "" &&
			($("#sobhyt").val().length == 15 || $("#sobhyt").val().length == 10)
		)
			setTimeout(checkthe, 500);
	});
	...
	$.get(
		window.location.origin + "/web_his/Cau_Hinh_Tham_So_XuatXMLBHYT",
		function (t) {
			(taikhoan = $(t).find("#motathamso123").val()),
				(matkhau = md5($(t).find("#motathamso124").val()));
		}
	);
						...
						if (d.maThe == null) {
							...
							if (d.maKetQua == "070") {
								switch (d.ngaySinh.length) {
									case 4:
										$("#chinamsinh").prop("checked", true);
										$("#namsinh").val(
											"01/01/" + d.ngaySinh
										);
										tempngaysinh = "01/01/" + d.ngaySinh;
										break;
									case 7:
										$("#chinamsinh").prop("checked", true);
										$("#namsinh").val("01/" + d.ngaySinh);
										tempngaysinh = "01/" + d.ngaySinh;
										break;
									case 10:
										$("#chinamsinh").prop("checked", false);
										$("#namsinh").val(d.ngaySinh);
										tempngaysinh = d.ngaySinh;
										break;
									default:
										editColorTrue("#namsinh", 1);
								}
							...
							if (
								d.maKetQua == "003" &&
								!$("#sobhyt").prop("disabled") &&
								d.maDKBD == d.maDKBDMoi
							) {
								var p3 =
							...
							if (d.dsLichSuKCB2018 != null) {
								ls = dichlichsu(
									d.dsLichSuKCB2018[0].maCSKCB,
									d.dsLichSuKCB2018[0].ngayRa,
									d.dsLichSuKCB2018[0].tinhTrang,
									d.dsLichSuKCB2018[0].kqDieuTri,
									d.dsLichSuKCB2018[0].tenBenh
								);
								lichsu_msg = ls.msg;
								lichsu_num +=
									"LSKB(" + d.dsLichSuKCB2018.length + ")";
								if (!ls.kq)
									$("#mod_bhyt_lichsu_msg").css(
										"color",
										"red"
									);
								...
								for (
									i = 0;
									i < d.dsLichSuKCB2018.length && i < 10;
									i++
								) {
									lichsu_kcb += dichlichsuCT(
										d.dsLichSuKCB2018[i].maCSKCB,
										d.dsLichSuKCB2018[i].ngayVao,
										d.dsLichSuKCB2018[i].tinhTrang,
										d.dsLichSuKCB2018[i].kqDieuTri,
										d.dsLichSuKCB2018[i].tenBenh,
										d.dsLichSuKCB2018[i].ngayRa,
										i + 1,
										dmndk[d.dsLichSuKCB2018[i].maCSKCB]
									);
								}
								//lichsu_kcb="";
							...
							if (
								d.dsLichSuKT2018 != null &&
								d.dsLichSuKT2018.length > 0
							) {
								lichsu_num +=
									"LSKT(" + d.dsLichSuKT2018.length + ")";
								for (i = 0; i < d.dsLichSuKT2018.length; i++) {
									lichsu_kt += dichlichsuKT(
										d.dsLichSuKT2018[i].userKT,
										d.dsLichSuKT2018[i].thoiGianKT,
										d.dsLichSuKT2018[i].maLoi,
										d.dsLichSuKT2018[i].thongBao,
										i + 1
									);
								}
							...
							$("#mod_bhyt_lichsu_msg").html(
								lichsu_msg + lichsu_num + "</span>"
							);
							if (d.dsLichSuKCB2018 != null) {
								$("#tencskcbls").prop(
									"title",
									dmndk[d.dsLichSuKCB2018[0].maCSKCB]
								);
							}
							$("#hienlichsu").on("click", function () {
								$("#mod_lichsuct").show();
								$("body").css("overflow", "hidden");
							});
							$("#mod_lichsuct").on("click", function () {
								$("#mod_lichsuct").hide();
							...
							if (!$("#hoten").prop("disabled")) {
								...
								$("#tungay").val() != d.gtTheTu
									? editColorTrue("#tungay", 0)
									: editColorTrue("#tungay", 1);
								$("#denngay").val() != d.gtTheDen
									? editColorTrue("#denngay", 0)
									: editColorTrue("#denngay", 1);
								$("#thoigian_du5namlientuc").val() !=
								d.ngayDu5Nam
									? editColorTrue(
								...
								$("#noidangky").val() != d.maDKBD
									? editColorTrue("#noidangky", 0)
									: editColorTrue("#noidangky", 1);
								$("#diachi").val() != d.diaChi
									? editColorTrue("#diachi", 0)
									: editColorTrue("#diachi", 1);
								$("#manoidoituong").val() != d.maKV
									? editColorTrue("#manoidoituong", 0)
									: editColorTrue("#manoidoituong", 1);
								if ("Nam" == d.gioiTinh) {
									$("#cbgioitinh").val() != "true" ||
									$("#gioitinh").val() != "1"
										? editColorTrue("#cbgioitinh", 0)
										: editColorTrue("#cbgioitinh", 1);
								} else {
									$("#cbgioitinh").val() != "false" ||
									$("#gioitinh").val() != "0"
										? editColorTrue("#cbgioitinh", 0)
								...
								switch (d.ngaySinh.length) {
									case 4:
										!$("#chinamsinh").prop("checked") ||
										$("#namsinh").val().substring(6, 10) !=
											d.ngaySinh
											? editColorTrue("#namsinh", 0)
											: editColorTrue("#namsinh", 1);
										break;
									case 7:
										!$("#chinamsinh").prop("checked") ||
										$("#namsinh").val().substring(4, 10) !=
											d.ngaySinh
											? editColorTrue("#namsinh", 0)
											: editColorTrue("#namsinh", 1);
										break;
...
function setColorTrue(value) {
	switch (value) {
		case 0: {
			editColorTrue("#sobhyt", 0);
			$("#part1").css("color", "red");
			break;
		}
		case 1: {
			editColorTrue("#hoten", 1);
		...
		case 2: {
			editColorTrue("#sobhyt", 2);
			$("#part1").css("color", "#a76e06");
			break;
		}
		case 3: {
			editColorTrue("#hoten", 0);
			editColorTrue("#sobhyt", 1);
			$("#part1").css("color", "red");
			break;
		}
		default: {
			editColorTrue("#hoten");
			editColorTrue("#sobhyt");
			editColorTrue("#tungay");
			editColorTrue("#denngay");
			editColorTrue("#noidangky");
			editColorTrue("#cbgioitinh");
			editColorTrue("#namsinh");
			editColorTrue("#diachi");
			editColorTrue("#manoidoituong");
			$("#part1").css("color", "red");
			editColorTrue("#thoigian_du5namlientuc");
		}
	}
}
...
function editColorTrue(id, value) {
	switch (value) {
		case 0:
			$(id).css("background", "#fbd2d2");
			break;
		case 1:
			$(id).css("background", "#ddf5dd");
			break;
		case 2:
			$(id).css("background", "#f9d5a1");
			break;
		default:
			$(id).css("background", "");
	}
	if (!$("#sobhyt").prop("disabled")) {
		setTimeout(function () {
			$(id).css("background", "");
		}, 2000);
	}
...
function dichketqua(value) {
	switch (value) {
		case "000":
			return { kq: 1, msg: "Thẻ còn giá trị sử dụng" };
		case "001":
			return {
				kq: 2,
				msg:
		...
		case "003":
			return {
				kq: 2,
				msg: "Thẻ cũ hết giá trị sử dụng nhưng đã được cấp thẻ mới",
			};
		case "003.1":
			return { kq: 2, msg: "Thẻ đã được gia hạn" };
		case "004":
			return {
				kq: 1,
				msg: "Thẻ cũ còn giá trị sử dụng nhưng đã được cấp thẻ mới",
			};
		case "010":
			return { kq: 0, msg: "Thẻ hết giá trị sử dụng" };
		case "051":
			return { kq: 0, msg: "Mã thẻ không đúng" };
		case "052":
			return {
				kq: 0,
				msg: "Mã tỉnh cấp thẻ(kí tự thứ 4,5 của mã thẻ) không đúng",
			};
		case "053":
			return {
				kq: 0,
				msg: "Mã quyền lợi thẻ(kí tự thứ 3 của mã thẻ) không đúng",
			};
		case "050":
			return { kq: 0, msg: "Không thấy thông tin thẻ bhyt" };
		case "060":
			return { kq: 3, msg: "Thẻ sai họ tên" };
		case "061":
			return { kq: 3, msg: "Thẻ sai họ tên(đúng kí tự đầu)" };
		case "070":
			return { kq: 1, msg: "Thẻ sai ngày sinh" };
		case "100":
			return { kq: 0, msg: "Lỗi khi lấy dữ liệu sổ thẻ" };
		case "101":
			return { kq: 0, msg: "Lỗi server" };
		case "110":
			return { kq: 0, msg: "Thẻ đã thu hồi" };
		case "120":
			return { kq: 0, msg: "Thẻ đã báo giảm" };
		case "121":
			return {
				kq: 0,
				msg: "Thẻ đã báo giảm. Giảm chuyển ngoại tỉnh",
			};
		case "122":
			return {
				kq: 0,
				msg: "Thẻ đã báo giảm. Giảm chuyển nội tỉnh",
			};
		case "123":
			return {
				kq: 0,
				msg: "Thẻ đã báo giảm. Thu hồi do tăng lại cùng đơn vị",
			};
		case "124":
			return {
				kq: 0,
				msg: "Thẻ đã báo giảm. Ngừng tham gia",
			};
		case "130":
			return { kq: 2, msg: "Trẻ em không xuất trình thẻ" };
		case "205":
			return {
				kq: 0,
				msg: "Lỗi sai định dạng tham số truyền vào",
			};
		case "401":
			return { kq: 0, msg: "Lỗi xác thực tài khoản" };
		default:
			return { kq: 0, msg: "Không xác định" };
	}
}
function dichlichsu(cskcb, ngayra, tinhtrang, ketqua, tenbenh) {
	...
	dHT = y + m + d;
	tenbenh.length > 45
		? (chidinh = tenbenh.substring(0, 42) + "...)")
		: (chidinh = tenbenh + ")");
	if (dHT == ngayra.substring(0, 8)) {
		return {
			msg:
				"<b>BN đã khám trong ngày lúc " +
				ngayra.substring(8, 10) +
				":" +
				ngayra.substring(10, 12) +
				"! </b>" +
				"(<b style='color: #a76e06;' id='tencskcbls'>" +
				cskcb +
				"</b> | " +
				kq +
				" | " +
		...
		return {
			msg:
				"<b>Lần khám gần nhất " +
				ngayra.substring(6, 8) +
				"/" +
				ngayra.substring(4, 6) +
				"/" +
				ngayra.substring(0, 4) +
				"</b> (<b style='color: #a76e06;' id='tencskcbls'>" +
				cskcb +
				"</b> | " +
				kq +
				" | " +
...
function dichlichsuCT(
	...
	rs =
		...
		stt +
		"</td><td>" +
		ngayvao.substring(6, 8) +
		"/" +
		ngayvao.substring(4, 6) +
		"/" +
		ngayvao.substring(0, 4) +
		" " +
		ngayvao.substring(8, 10) +
		":" +
		ngayvao.substring(10, 12) +
		"</td>";
	rs +=
		"<td>" +
		ngayra.substring(6, 8) +
...
function dichlichsuKT(cskcb, thoigian, mathongbao, thongbao, stt) {
	...
	rs +=
		"<td>" +
		thoigian.substring(6, 8) +
		"/" +
		thoigian.substring(4, 6) +
		"/" +
		thoigian.substring(0, 4) +
		" " +
		thoigian.substring(8, 10) +
		":" +
		thoigian.substring(10, 12) +
		"</td>";
